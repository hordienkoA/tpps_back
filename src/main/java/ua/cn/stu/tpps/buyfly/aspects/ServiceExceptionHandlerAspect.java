...
import static ua.cn.stu.tpps.buyfly.values.ResourceStringsDictionary.SERVICES_EXCEPTION_THROWN;
...
public class ServiceExceptionHandlerAspect {
    ...
    @Around("execution(* ua.cn.stu.tpps.buyfly.services.implemented..*.*(..))")
    public Object exceptionHandlerWithReturnType(ProceedingJoinPoint joinPoint) throws Throwable {
        try {
            ...
            logger.error(ResourceMessage.get(BUNDLE_EXCEPTION_MESSAGES, SERVICES_EXCEPTION_THROWN, className, methodName, argumentsString), ex);
